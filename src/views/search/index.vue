    ...
    <mt-search class="search" v-model="value" placeholder="搜索歌曲、歌手、专辑" cancel-text="x">
      <div class="search-list">
        ...
        <mt-loadmore :top-method="loadTop" :bottom-method="loadBottom" :bottom-all-loaded="allLoaded" ref="loadmoreNode">
          <ul class="song-list">
            <li>
              <router-link :to="`/artist/${searchArtistData.id}`">
                <div class="artist">
...
export default {
  ...
  methods: {
    ...
    getSearch(keywords, type = 1, offset = 0) {
    ...
    loadTop() {
      console.log(111)
      this.getSearch(this.value, 1, 0) //单歌
      this.getSearch(this.value, 100, 0) //歌手
      this.getSearch(this.value, 10, 0) //专辑
      //这里写你代码，获取最新数据
    ...
    loadBottom() {
      console.log(222)
      this.pageNum++
      this.getSearch(this.value, 1, this.pageNum * 30) //单歌
      //这里写你代码，加载更多数据
  ...
  watch: {
    value(newval) {
      if (newval == '') return
      this.getSearch(newval, 1) //单歌
      this.getSearch(newval, 100) //歌手
      this.getSearch(newval, 10) //专辑
    }
  },
  components: {
    appNav
